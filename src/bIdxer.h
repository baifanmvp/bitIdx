...
bIdxer* bIdxer_new(char* bitPath, char* ip, int port)
{
    ...
    p_bidxer->peql = eqlClient_new(ip, port);
    return p_bidxer;
...
char* bIdxer_query(bIdxer* pbIdxer, char* query)
{
    ...
    cjson* jsoner = p_bidxer->jsoner;
    cjson_parse(jsoner, query);
    char* lp_op = jsoner->cjson_obj(jsoner, BIDX_JSONKEY_op)->obj_s(jsoner);
...
char* bIdxer_query_qryid(bIdxer* pbIdxer)
{
    ...
    size_t n_ids_cnt = p_ids_json->cjson_array_size(p_ids_json);
    size_t idx = 0;

    int code = 0;
    cjson* lp_res_json = cjson_new("{}");
    lp_res_json->sstr(lp_res_json, BIDX_JSONKEY_op, BIDX_JSONVAL_QRYID);
    lp_res_json->ssum(lp_res_json, BIDX_JSONKEY_code, 0);
    
    bIdxArray** lp_org_arrays = bIdxBasOp_searchOrg_array(pbIdxer->pBasOp, org);


//get id
    bIdxBasRes *p_id_res = NULL;
    size_t n_real_cnt = 0;
    IDXIDRES_INIT(p_id_res, n_ids_cnt)
    while(idx < n_ids_cnt)
    {
        cjson* p_ids_tmp = p_ids_json->ref(p_ids_json);    
        char* lp_id = p_tags_tmp->obj_i(p_tags_tmp, idx)->obj_s(p_tags_tmp);
        bid_t id = atoi(lp_id);
...
//add tag
    ...
    lp_res_json->ssum(lp_res_json, BIDX_JSONKEY_code, code);
    char* lp_ret_str = lp_res_json->obj_s(lp_res_json);
...
char* bIdxer_query_utagcnd(bIdxer* pbIdxer)
{
    ...
    char* lp_eql_query = jsoner->rs(jsoner)->obj(jsoner, BIDX_JSONKEY_matchEql)->ref(jsoner);
...
type_t bIdxer_get_needFields(bIdxer* pbIdxer)
{
    cjson* jsoner = p_bidxer->jsoner;
    type_t n_tag_type = 0;
    char* p_needDoc = jsoner->rs(jsoner)->obj(jsoner, BIDX_JSONVAL_needDoc);
    
    if(p_needDoc && strcmp(p_needDoc, "true") == 0)
    {
        char* p_needFields = jsoner->rs(jsoner)->obj(jsoner, BIDX_JSONVAL_needFields);
...
char* bIdxer_get_labelTag(bIdxer* pbIdxer, bIdxArray** pLoopArrays, bid_t id, char* label)
{
    ...
    while(*pLoopArrays)
    {
        size_t n_all_len = strlen((*pLoopArrays)->allKey);
        
        if( (p_iter_label + n_all_len) < (label + 4096 -2)
            && n_all_len > strlen(BIDX_JSONKEY_LABEL)
            && strncmp((*pLoopArrays)->allKey, BIDX_JSONKEY_LABEL, 5) == 0)
        {
            byte val ;
            bIdxArray_get_bitVal((*pLoopArrays), id, val);
            if(val)
            {
                char* p_real_tag  = strchr((*pLoopArrays)->allKey, '.') + 1;
                memcpy(p_iter_label, p_real_tag, n_all_len - 6);
                p_iter_label += n_all_len - 6;
                *p_iter_label = ',';
                p_iter_label ++;
            }
        }    
...
char* bIdxer_get_otherTag(bIdxer* pbIdxer, char* org , char* tag, bid_t id, char* tagbuf)
{
    ...
    if(lp_array)
    {
        ...
        if(val)
        {
            ...
            memcpy(p_iter_tag, p_real_tag, 1);
            p_iter_tag ++;
            *p_iter_tag = ',';
            p_iter_tag ++;
        }
...
{
        //get tag info
    char* org = jsoner->rs(jsoner)->obj(jsoner, BIDX_JSONVAL_org)->obj_s(jsoner);
    ...
    cjson* lp_record_json = cjson_new("{record:[]}");
    char label[4096];
    char read[64];
    char star[64];
    char imp[64];
    ...
    while(idx < n_ids_cnt)
    {
        ...
        *p_iter_imp = '\0';
        ...
        lp_record_json->sobj_i(lp_record_json, lp_record_json->ar_sz(lp_record_json), lp_id_json);
